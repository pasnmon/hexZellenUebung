...
public class HexLayout implements LayoutManager {
    ...
    public Dimension preferredLayoutSize(Container parent)
    {
        synchronized ( parent.getTreeLock() )
        {
            ...
            int dx = insets.left + insets.right + (ncols*2) * w + (ncols - 1) * cgap;
            int dy = (int)((insets.top + insets.bottom + (nrows*1.5) *h + (nrows-1))*0.75);
    ...
    public Dimension minimumLayoutSize(Container parent)
    {
        synchronized ( parent.getTreeLock() )
        {
            ...
            int dx = insets.left + insets.right + (ncols*2) * w + (ncols - 1) * w;
    ...
    public void layoutContainer(Container parent)
    {
        synchronized (parent.getTreeLock())
        {
            ...
            float effectiveColls = 1 + (((ncols*2)-1) * 0.75f);
            w = (int)((w - (ncols - 1) * cgap) / effectiveColls); // width for components

            //float effectiveRows = 1 + ((nrows - 1) * 0.75f);
            int h = (int)(w*1.15);              //height for components





            int xoffset = (w/4)*3; //x offset for staggered Rows (moves staggered rows 3/4 of the width to the right)
            int yoffset = (h /2);   //y offset (moves each row 1/2 height down)
            boolean staggeredRow = false;
            for ( int r = 0, y = insets.top; r < nrows; r++, y += yoffset) // as long as r < nrow, and also adds the height of the rows
            {
                int offset = 0;

                if ( staggeredRow )     //sets the offset if it is a staggeredRow
                    offset = xoffset;

                for ( int c = 0, x = insets.left; c < ncols; c++, x += (w+(w/2)))   // x is the horizontal offset between each component
                {
                    int i = r * ncols + c;      // calcs the number of the component

                    if ( i < ncomponents )      // if the number of the component < the amount of the components
                    {
                        parent.getComponent(i).setBounds(x+offset, y, w, h);    //place the component
                    }
